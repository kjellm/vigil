...
describe Vigil do
  ...
  context "When no VM has been built before" do
    it "builds a VM from scratch" do
      @shell.should_receive('exists?').with("#@base/run/znork/boxes/znork-1_complete.pkg").and_return(false)

      @shell.should_receive('exists?').with("#@base/run/znork/boxes/znork-1.box").and_return(false)
  ...
  context "When a VM has been completely built for the previous revision" do
    ...
    context "and none of the VM configuration files has changed" do
      it "reuses the VM" do
        @shell.should_receive('exists?').with("#@base/run/znork/boxes/znork-0.box").and_return(true)
        @shell.should_receive('exists?').with("#@base/run/znork/boxes/znork-0_no_gems.pkg").and_return(true)
        @shell.should_receive('exists?').with("#@base/run/znork/boxes/znork-0_complete.pkg").and_return(true)
        @shell.should_receive('exists?').with("#@base/run/znork/boxes/znork-1.box").and_return(false)
        @shell.should_receive('exists?').with("#@base/run/znork/boxes/znork-1_no_gems.pkg").and_return(false)

        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- definitions").and_return(true)
        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- manifests").and_return(true)
        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- Gemfile*").and_return(true)

        @shell.should_receive('ln').with("#@base/run/znork/boxes/znork-0.box", "#@base/run/znork/boxes/znork-1.box").ordered
        @shell.should_receive('ln').with("#@base/run/znork/boxes/znork-0_no_gems.pkg", "#@base/run/znork/boxes/znork-1_no_gems.pkg").ordered
        @shell.should_receive('ln').with("#@base/run/znork/boxes/znork-0_complete.pkg", "#@base/run/znork/boxes/znork-1_complete.pkg").ordered
      end
    end
    ...
    context "and only the veewee definitions has changed" do
      it "builds the VM from scratch" do
        ...
        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- definitions").and_return(false)
    ...
    context "and only the puppet manifests has changed" do
      it "uses the previous revisions basebox to build the VM" do
        ...
        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- definitions").and_return(true)
        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- manifests").and_return(false)
    ...
    context "and only Gemfile* has changed" do
      it "uses the previous revisions basebox to build the VM" do
        ...
        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- definitions").and_return(true)
        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- manifests").and_return(true)
        @shell.should_receive('__system').with("git diff --quiet HEAD^ -- Gemfile*").and_return(false)
